...
@property (weak, nonatomic) UIButton *codenextBtn,*fortgotBtn,*nextBtn;
...
- (void)setUpViews {
    ...
    /*
    UIButton *codenextBtn = [[UIButton alloc]init];
    [codenextBtn setTitle:@"验证码登录" forState:UIControlStateNormal];
    codenextBtn.titleLabel.font = [UIFont fontWithName:regularFont size:16];
    [codenextBtn setTitleColor:[UIColor colorWithHexString:@"#999999"] forState:UIControlStateNormal];
    [codenextBtn addTarget:self action:@selector(goTocodeLogin) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:codenextBtn];
    self.codenextBtn = codenextBtn;
...
//    [self.codenextBtn mas_makeConstraints:^(MASConstraintMaker *make) {
...
//    }];
    ...
    [self.lineTree mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(weakSelf.nextBtn.mas_bottom).offset(135);
        make.left.mas_equalTo(weakSelf).offset(46);
        make.size.mas_equalTo(CGSizeMake((ScreenWidth-212)/2,1));
    }];
    
    [self.socialLogin mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(weakSelf.nextBtn.mas_bottom).offset(123);
